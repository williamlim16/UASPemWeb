            ...
            <div class="col-9">
                    ...
                    <div class="form-group">
                        ...
                        <select id="screeningPick" name="screeningPick" class="custom-select">
                            @foreach($screenings as $s)
                                <option
                                    value='{{$s->sId.",".$s->aId}}' @if($loop->index == 0){{"selected"}}@endif>{{"[".$s->sId."] Showing: ".$s->title." @ ".$s->date.", ".$s->time.". Auditorium: ". $s->aName}}</option>
                            @endforeach
                        </select>
                ...
                <form action="{{route('tickets.store')}}" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <select id="seat_id" name="seat_id" class="custom-select ml-1 dis" disabled required></select>
                    </div>
                    <div class="form-group">
                        ...
                        <select id="user_id" name="user_id" class="custom-select">
                            @foreach($users as $u)
                                <option
                                    value='{{$u->id}}' @if($loop->index == 0){{"selected"}}@endif>{{"[".$u->id."] ".$u->name." | ".$u->email}}
                                </option>
                            @endforeach
                        </select>
                    </div>
